    ...
    for sentence in sentences:  # Iterate through each sentence in the list
        if sentence:  # Check if sentence is not empty
            audio_tensor = pipe.generate(
                sentence
            )  # Generate audio tensor for the sentence using the pipeline
            audio_np = (audio_tensor.cpu().numpy() * 32767).astype(
                np.int16
            )  # Convert tensor to numpy array, scale, and typecast
            if len(audio_np.shape) == 1:  # Check if the numpy array is 1D
                audio_np = np.expand_dims(
                    audio_np, axis=0
                )  # Make the numpy array 2D for compatibility
            else:
                audio_np = (
                    audio_np.T
                )  # Transpose the array to match expected input shape
...
def play_audio_from_queue(audio_queue):
    while True:  # Continuously attempt to play audio from the queue
        audio_np = (
            audio_queue.get()
        )  # Get the next numpy array (audio data) from the queue
...
def start_processing():
    ...
    sentences = sent_tokenize(
        user_input
    )  # Use NLTK's sent_tokenize to define a sentence
    ...
    processing_thread = threading.Thread(
        target=process_text_to_audio, args=(sentences, pipe)
    )  # Set up thread for text-to-audio processing
    playback_thread = threading.Thread(
        target=play_audio_from_queue, args=(audio_queue,)
    )  # Set up thread for audio playback
...
process_button = Button(
    root, text="Text to Speech", command=start_processing, bg=button_bg, fg=text_fg
)
process_button.grid(row=1, column=0, sticky="ew", padx=10)

file_button = Button(
    root, text="Extract Text from File", command=select_file, bg=button_bg, fg=text_fg
)
file_button.grid(row=2, column=0, sticky="ew", padx=10)

support_label = Label(
    root,
    text="Supports .pdf (with OCR already done on them), .docx, and .txt files.",
    bg=label_bg,
    fg=text_fg,
)
