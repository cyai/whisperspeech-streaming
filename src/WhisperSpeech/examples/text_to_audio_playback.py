    ...
    for sentence in sentences:  # Iterate through each sentence
        if sentence:  # Ensure the sentence is not empty
            audio_tensor = pipe.generate(
                sentence
            )  # Generate audio tensor for the sentence
...
def play_audio_from_queue(audio_queue):
    while True:  # Loop indefinitely to process audio data
        audio_np = (
            audio_queue.get()
        )  # Retrieve the next audio numpy array from the queue
        if (
            audio_np is None
        ):  # Check if the queue is signaling that processing is complete
            break  # Exit the loop if signal received
        try:
            sd.play(
                audio_np, samplerate=24000
            )  # Play the audio numpy array using sounddevice
            sd.wait()  # Wait for the playback to finish before proceeding
        except Exception as e:
            print(
                f"Error playing audio: {e}"
            )  # Print any errors encountered during playback
