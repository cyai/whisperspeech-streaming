...
def load_dataset(
    ...
    from WhisperSpeech.whisperspeech import utils, languages

    dataset_dir = Path(dataset_dir)
    shards = utils.shard_glob(dataset_dir / "encodec-3kbps/*.tar.gz")
    with open(dataset_dir / "atoks-samples.list") as f:
    ...
    excludes = (
        ...
        if not validation and exclude_datasets
        else set()
    )

    def check_for_nan(s):
        if torch.tensor(s["spk_emb.npy"]).isnan().any():
            print("found NaN:", s["__key__"])
        return s
    ...
    randomized_conditionings = (
        set(randomized_conditionings.split(",")) if randomized_conditionings else set()
    )
    rng = np.random.default_rng()

    def randomize_conditionings(batch):
        if not randomized_conditionings:
            return batch
        bs = len(batch)
    ...
    ds = wds.WebDataset(
        shards, resampled=not validation, nodesplitter=same_on_all_nodes
    ).compose(
        wds.decode(),
        ...
        wds.map_dict(
            **{"spk_emb.npy": np.nan_to_num}
        ),  # remove nans from the speaker embedding model
...
class CategoricalEmbedding(nn.Module):
    ...
    def forward(self, x):
        x[torch.isnan(x)] = (
            self.codes
        )  # separate code for NaNs which represent missing conditioning
...
class BinnedEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        qx[torch.isnan(x)] = (
            self.bins
        )  # separate bin for NaNs which represent missing conditioning
...
class SpeakerEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.spk_to_hidden:
            x = self.spk_to_hidden(x.to(self.spk_to_hidden.weight.dtype))
        return x
...
class SADelARTransformer(nn.Module):
    ...
    def run_encoder(self, Stoks, conds):
        ...
        cond_embs = torch.zeros(
            (bs, semb.shape[-1]), dtype=semb.dtype, device=semb.device
        )
        for k in self.cond_embeddings.keys():
            ...
            if isinstance(c, np.ndarray):
                c = torch.tensor(c, device=Stoks.device)
            if isinstance(c, torch.Tensor):
                c = c.to(device=Stoks.device)
...
def make_model(
    ...
    from WhisperSpeech.whisperspeech import vq_stoks
