...
class SimpleVisual:
    ...
    def add_table_row(self, it, avg_train_loss, val_loss):
        ...
        self.masterbar.write(
            [
                it,
                f"{avg_train_loss:.5f}",
                f"{val_loss:.5f}",
                fastprogress.core.format_time(elapsed_t),
            ],
            table=True,
        )
    ...
    def on_iter(self, bar, it, avg_train_loss, val_loss):
        ...
        bar.comment = (
            f"#{epoch}/{self.epochs} loss: {avg_train_loss:.3f} / {val_loss:.3f}"
        )
...
def validate(
    model, val, half=True, bs=16, drop_last=False, dl_workers=8, device="cuda"
):
    if isinstance(val, torch.utils.data.IterableDataset):
        val_loader = (
            wds.WebLoader(
                val, batch_size=None, num_workers=dl_workers, drop_last=drop_last
            )
            .unbatched()
            .shuffle(1024)
            .batched(bs)
...
def train(
    ...
    dl_workers=8,
    visual_class=SimpleVisual,
    profiler=None,
    run_valid_every_iters=8000,
    table_row_every_iters=80000,
    chkpt_every_iters=None,
    device="cuda",
    ...
    if isinstance(train, torch.utils.data.IterableDataset):
        ...
        val_loader = (
            ...
            .unbatched()
            .batched(bs)
            .with_length(val.total_samples // bs)
        )
    else:
    ...
    try:
        ...
        optimizer = torch.optim.AdamW(
            lr=lr, betas=(0.9, 0.95), fused=device != "cpu", params=groups
        )
            ...
            for args in bar:
                ...
                if it >= next_val_it:
                    ...
                    with record_function("plotting"):
                        visual.add_data(
                            it, lr_scheduler.get_last_lr(), avg_train_loss, val_loss
                        )
