...
def load_dataset(
    ...
    for name in ["small.en-txt", "medium-txt"]:
        if (dataset_dir / name).exists():
            ...
            break
    if txt_dir is None:
        raise ArgumentError(f"No transcripts found in {dataset_dir}")
    ...
    if not language and model.endswith("en"):
        language = "en"
    ...
    ds = vad_merge.chunked_audio_dataset(
        shards, "raw", resampled=not validation, nodesplitter=same_on_all_nodes
    ).compose(
...
from WhisperSpeech.whisperspeech.train import *
