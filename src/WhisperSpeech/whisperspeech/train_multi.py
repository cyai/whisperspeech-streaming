...
class TrainingTask(pl.LightningModule):
    ...
    def on_fit_start(self):
        ...
        if self.model_hparams["torch_compile"] and getattr(
    ...
    def configure_optimizers(self):
        ...
        if self.model_hparams["lr_schedule"] == "cosine":
            ...
            train_scheduler = torch.optim.lr_scheduler.MultiStepLR(
                optimizer,
                [int(total_steps - warmup_steps - 0.1 * total_steps)],
                1 / 8,
            )
            lr_scheduler = torch.optim.lr_scheduler.SequentialLR(
                optimizer,
    ...
    def validation_step(self, val_batch, batch_idx, dataloader_idx=0):
        ...
        if hasattr(self.model, "get_metrics"):
            self.log_dict(
                {f"metrics/{k}_{name}": v for k, v in self.model.get_metrics().items()},
                sync_dist=True,
                add_dataloader_idx=False,
            )
    ...
    def on_validation_epoch_end(self):
        for name, weight in zip(train_dss_names, train_weights):
            self.log(
                f"trainer/{name}-batches",
                weight.to(self.device) * self.global_step,
                sync_dist=True,
            )
...
def parse_and_call(name, fun, args, kwargs={}, log_to_wandb=True):
    ...
    if (
        log_to_wandb
        and type(wandb_logger.experiment.config) == wandb.sdk.wandb_config.Config
    ):
        wandb_logger.experiment.config[name] = {
            k: v for k, v in args.items() if k not in ["dataset", "tunables"]
        }
    return fun(**args)
...
def simplify_folder_names(lst):
    ...
    ]  # normalize pathnames, removing trailing and leading slashes
...
parser.add_argument(
    "--input-dir", type=str, default="", help="input data path"
)  # fixed in the model for now
parser.add_argument(
    "--dataset-config", type=str, default="", help="common dataset options"
)
parser.add_argument(
    "--training-data", action="append", type=str, default=[], help="training dataset"
)
parser.add_argument(
    "--validation-data",
    action="append",
    type=str,
    default=[],
    help="validation dataset (can be passed multiple times)",
)
parser.add_argument(
    "--monitored-metric",
    type=str,
...
parser.add_argument(
    "--iterations", type=int, default=8000, help="total training iterations"
)
parser.add_argument(
    "--validate-every-n-steps",
    type=int,
    default=500,
    help="how training steps to run between validations",
)
parser.add_argument(
    "--weight-decay", type=float, default=1e-2, help="optimizer weight decay"
)
parser.add_argument(
    "--lr0", type=float, default=1e-4, help="optimizer initial learning rate"
)
parser.add_argument(
    "--lr-schedule",
    type=str,
    default="cosine",
    help="the learning rate schedule [cosine, linear or wsd]",
)
parser.add_argument(
    "--clip-gradient-norm",
    type=float,
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="perform the optimizer step only after going through several batches of samples",
)
parser.add_argument(
    "--precision", type=str, default="16-mixed", help="floating point precision"
)
parser.add_argument(
    "--torch-compile",
    type=bool,
    default=False,
    help="compile (parts of) the model with torch.compile",
)
parser.add_argument(
    "--warmup-steps",
    type=int,
    default=10000,
    help="total number steps during which the learning rate rises (defaults to 10k updates)",
)
...
parser.add_argument(
    ...
    type=Path,
    default=None,
    help="initialize the weights from the given model",
)
parser.add_argument(
    "--strategy", type=str, default="ddp", help="distributed training strategy"
)
parser.add_argument(
    "--wandb-suffix", type=str, default=None, help="W&B project name suffix"
)
parser.add_argument(
    "--wandb-task-name",
    type=str,
    default=None,
    help="Task name for the W&B project name",
)
...
train_dss_names = simplify_folder_names(
    [
        parse_dataset_string(train_ds_config)[0]
        for train_ds_config in args["training_data"]
    ]
)
...
val_dss_names = [
    parse_dataset_string(val_ds_config)[0] for val_ds_config in args["validation_data"]
]
...
val_dss = [
    parse_and_call(
        f"val_ds_{i}",
        task.load_dataset,
        parse_dataset_string(val_ds_config) + dataset_config,
        {"validation": True},
    )
...
val_loaders = [
    ...
    .unbatched()
    .batched(batch_size)
    .with_length(val_ds.total_samples // batch_size)
    for val_ds in val_dss
]

tunables = None
if hasattr(task, "Tunables"):
    tunables = parse_and_call(
        "tunables", task.Tunables, tunables_args, log_to_wandb=False
    )
...
with trainer.init_module(empty_init=True):
    if args["load_from"]:
        ...
        if tunables is not None:
            model_kwargs["tunables"] = tunables
...
trainer.fit(
    model=TrainingTask(model, model_hparams=hyp_params),
