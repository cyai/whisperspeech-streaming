...
def load_dataset(
    ...
    with open(dataset_dir / "txt-samples.list") as f:
    ...
    ds = wds.WebDataset(
        ...
        wds.select(
            lambda s: s["__key__"] not in excludes and len(s["stoks.npy"]) > 0
        ),  # discard validation samples
        ...
        wds.to_tuple(
            "in_ttoks", "out_ttoks", "language", "cps", "in_stoks", "out_stoks"
        ),
    )
    if validation:
        ds = ds.compose(wds.batched(samples)).slice(1)
    else:
        ds = ds.compose(wds.shuffle(20000, initial=20000), wds.batched(2048))
