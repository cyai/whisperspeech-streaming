...
def split_dataset(
    ...
    outputs = {
        k: make_tar_writer(Path(k).parent / mode / (Path(k).name + suffix))
        for k in splits
    }
    ...
    print(
        f"Generating splits: {' '.join(outputs.keys())}, looking for {len(needles)} {mode} samples..."
    )
    ...
    for s in progress_bar(dl, total="noinfer"):
        if mode == "mvad":
            ...
            for i in chunks[s["__key__"]]:
                mask[i] = True
            new = {}
            for k in [
                "__key__",
                mvad_kind + ".vad.npy",
                mvad_kind + ".spk_emb.npy",
                mvad_kind + ".subvads.pyd",
                "gain_shift.npy",
            ]:
                v = s[k]
